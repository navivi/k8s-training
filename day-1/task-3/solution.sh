...
curl-each-node(){
  ...
  for i in `seq 3`; do 
    echo -n "\$ curl --write-out %{http_code} --silent --output /dev/null kube-node-$i:$web_node_port/media/favicon.ico"
    read text
    RESULT=$(curl --write-out %{http_code} --silent --output /dev/null kube-node-$i:$web_node_port/media/favicon.ico)
    echo $RESULT
    echo "---------------------------------------------------"
  done
}
