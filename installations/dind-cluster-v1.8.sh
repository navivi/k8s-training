...
EMBEDDED_CONFIG=y;DIND_IMAGE=navivi/kubeadm-dind-cluster:v1.8
...
function dind::run {
  ...
  local p_port_80
  PUBLISH_PORT_80=${PUBLISH_PORT_80:-true}
  if [[ ${PUBLISH_PORT_80} ]]; then
    p_port_80=$([[ `echo "${container_name}" | awk -F '-' '{print $3}'` == 1 ]] && echo "--publish 80:80")
  fi
  ...
  docker run \
	 -e IP_MODE="${IP_MODE}" \
         ...
         --name "${container_name}" \
         --hostname "${container_name}" ${p_port_80} \
...
function dind::init-helm {
  dind::step "Initiating Helm"
  dind::retry helm init

  local tiller_ready
  local ntries=50
  while true; do
    if dind::component-ready app=helm; then
      tiller_ready=y
    else
      tiller_ready=
    fi
    if [[ ${tiller_ready} ]]; then
      echo "[done]" >&2
      break
    fi
    if ((--ntries == 0)); then
      echo "Error waiting for tiller" >&2
      exit 1
    fi
    echo -n "." >&2
    sleep 1
...
function dind::deploy-nginx-ingress {
  dind::step "Deploying Nginx-Ingress"
  dind::retry helm install --name ing --namespace kube-system stable/nginx-ingress --set controller.hostNetwork=true --set controller.daemonset.useHostPort=true --set controller.kind=DaemonSet --set controller.service.type="" --set rbac.create=true

  local nginx_ready
  local ntries=200
  #local n=3
  while true; do
    if dind::component-ready app=nginx-ingress; then
      nginx_ready=y
    else
      nginx_ready=
    fi
    ...
    if ((--ntries == 0)); then
      echo "Error waiting for nginx-ingress" >&2
      exit 1
    fi
    echo -n "." >&2
    sleep 1
...
case "${1:-}" in
  up)
    if [[ ! ( ${DIND_IMAGE} =~ local ) ]]; then
      ...
      #nesia docker pull "${DIND_IMAGE}" >&2
    fi
